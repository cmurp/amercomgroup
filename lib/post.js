...
router.post('/quotes', (req, res) => {
	...
	res.render('email/request', { layout: null } , function(err,html){
		...
		mail.sendMail('Michael Gibson', process.env.EMAIL, process.env.EMAIL, 'Someone requested a quote!', html, req.body);
		res.redirect('/');
    });
});
...
router.post('/subscribe', (req, res) => {
	res.render('email/thank-you', { layout: null } , function(err,html){
        if( err ) console.log('error in email template');
        mail.sendMail('Michael Gibson', process.env.EMAIL, process.env.EMAIL, 'New subscriber!', html, req.body.email + " would like to keep up to date with AmerCom.");
		res.redirect('/');
    });
});
...
router.post('/contact', (req, res) => {
	res.render('email/contact', { layout: null} , function(err,html){
        if( err ) console.log('error in email template');
        mail.sendMail(req.body.name, process.env.EMAIL, process.env.EMAIL, req.body.subject, html, req.body.message);
		res.redirect('/');
    });
});
...
router.post('/post', (req, res) => {
	if(req.body.title == '' || req.body.message == '' || req.body.file == '' || req.body.title == undefined || req.body.message == undefined || req.body.file == undefined){
		...
		console.log('title:' + req.body.title + 'post:' + req.body.message + 'thumbnail:' + req.body.file);
...
router.post('/delete', (req, res) => {
	var Blog = require('../models/Blog.js');
	Blog.findOne({title: req.body.blog},'post',function(err, item){
		if (err){
			res.send(false);
		...
		res.render('email/deleted', { layout: null } , function(err,html){
			if( err ) console.log('error in email template');
			mail.sendMail('Chris Murphy', process.env.EMAIL, process.env.EMAIL, 'BLOG DELETED: ' + req.body.blog, html, item.post);
		});
	});
